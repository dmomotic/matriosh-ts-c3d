...
export class MenorIgualQue extends NodoAST {
  linea: string;
  op_izq: NodoAST;
  op_der: NodoAST;
  ...
  traducir(ts: TablaSimbolos) {
    ...
    //Validaciones
    if (!control_izq) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando izquierdo de la operacion menor igual que` }));
      return;
    }
    if (!control_der) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando derecho de la operacion menor igual que` }));
      return;
    }
    ...
    if (tipo === TIPO_DATO.NULL) {
      ...
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar una operacion menor igual que con los tipos ${getNombreDeTipo(control_izq.tipo)} y ${getNombreDeTipo(control_der.tipo)}` }));
      return;
    }
    ...
    switch (tipo) {
      case TIPO_DATO.BOOLEAN:
        ...
        Codigo3D.addComentario('Operacion menor igual que');
