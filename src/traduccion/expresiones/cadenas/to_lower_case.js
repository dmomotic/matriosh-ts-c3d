...
class ToLowerCase extends nodoAST_1.NodoAST {
    ...
    traducir(ts) {
        ...
        if (this.id && !this.exp) {
            ...
            if (!variable) {
                errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede efectuar la operacion TO_LOWER_CASE en el id: ${this.id} ya que no existe en la tabla de simbolos` }));
                return;
            }
            //Si no es de tipo string
            if (!tipos_1.isTipoString(variable.tipo)) {
                errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar la operacion TO_LOWER_CASE en el id: ${this.id} por que es de tipo ${tipos_1.getNombreDeTipo(variable.tipo)}` }));
                return;
            }
            codigo3D_1.Codigo3D.addComentario(`Traduccion instruccion TO_LOWER_CASE`);
        ...
        //Si solo es una exp
            ...
            if (!control_cadena) {
                errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo traducir la expresion para la operacion TO_LOWER_CASE` }));
                return;
            }
            //Si no es de tipo string o array
            if (!tipos_1.isTipoString(control_cadena.tipo)) {
                errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar la operacion TO_LOWER_CASE por que la expresion es de tipo ${tipos_1.getNombreDeTipo(control_cadena.tipo)}` }));
                return;
            }
            codigo3D_1.Codigo3D.addComentario(`Traduccion instruccion TO_LOWER_CASE`);
            //REMUEVO EL TEMPORAL
            ...
            errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `Algo salio mal al realizar operacion TO_LOWER_CASE` }));
            return;
        }
        ...
        const uA = temporal_1.Temporal.getSiguiente();
        codigo3D_1.Codigo3D.add(`${uA} = ${'A'.charCodeAt(0)}; //A`);
        const uZ = temporal_1.Temporal.getSiguiente();
        codigo3D_1.Codigo3D.add(`${uZ} = ${'Z'.charCodeAt(0)}; //Z`);
        ...
        codigo3D_1.Codigo3D.add(`if(${char} < ${uA} ) goto ${lbl_false};`);
        codigo3D_1.Codigo3D.add(`if(${char} > ${uZ} ) goto ${lbl_false};`);
