...
export class ToUpperCase extends NodoAST {
  linea: string;
  id: string;
  exp: NodoAST;
  ...
  traducir(ts: TablaSimbolos) {
    ...
    if (this.id && !this.exp) {
      ...
      if (!variable) {
        Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede efectuar la operacion TO_UPPER_CASE en el id: ${this.id} ya que no existe en la tabla de simbolos` }));
        return;
      }
      //Si no es de tipo string
      if (!isTipoString(variable.tipo)) {
        Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar la operacion TO_UPPER_CASE en el id: ${this.id} por que es de tipo ${getNombreDeTipo(variable.tipo)}` }));
        return;
      }

      Codigo3D.addComentario(`Traduccion instruccion TO_UPPER_CASE`);
    ...
    //Si solo es una exp
      ...
      if (!control_cadena) {
        Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo traducir la expresion para la operacion TO_UPPER_CASE` }));
        return;
      }
      //Si no es de tipo string o array
      if (!isTipoString(control_cadena.tipo)) {
        Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar la operacion TO_UPPER_CASE por que la expresion es de tipo ${getNombreDeTipo(control_cadena.tipo)}` }));
        return;
      }
      ...
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `Algo salio mal al realizar operacion TO_UPPER_CASE` }));
      return;
    }
    ...
    const a = Temporal.getSiguiente();
    Codigo3D.add(`${a} = ${'a'.charCodeAt(0)}; //a`);
    const z = Temporal.getSiguiente();
    Codigo3D.add(`${z} = ${'z'.charCodeAt(0)}; //z`);
    ...
    Codigo3D.add(`if(${char} < ${a} ) goto ${lbl_false};`);
    Codigo3D.add(`if(${char} > ${z} ) goto ${lbl_false};`);
