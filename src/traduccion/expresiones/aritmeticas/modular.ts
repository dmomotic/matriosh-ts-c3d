...
export class Modular extends NodoAST {
  linea: string;
  op_izq: NodoAST;
  op_der: NodoAST;
  ...
  traducir(ts: TablaSimbolos) {
    ...
    //Validaciones
    if (!control_izq) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando izquierdo de la operacion Modulo` }));
      return;
    }
    if (!control_der) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando derecho de la operacion Modulo` }));
      return;
    }
    ...
    if (tipo === TIPO_DATO.NULL) {
      ...
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar una operacion Modulo con los tipos ${getNombreDeTipo(control_izq.tipo)} y ${getNombreDeTipo(control_der.tipo)}` }));
      return;
    }
    ...
    switch (tipo) {
      case TIPO_DATO.FLOAT:
        Codigo3D.addComentario('Modulo con resultado double');
        Codigo3D.add(`${temporal} = fmod(${control_izq.temporal},${control_der.temporal});`);
        return new Control({ temporal, tipo });
      case TIPO_DATO.INT:
        Codigo3D.addComentario('Modulo con resultado int');
        Codigo3D.add(`${temporal} = fmod(${control_izq.temporal},${control_der.temporal});`);
        return new Control({ temporal, tipo: TIPO_DATO.FLOAT }); //Asignado por seguridad
    }
  }
