...
export class Multiplicacion extends NodoAST {
  linea: string;
  op_izq: NodoAST;
  op_der: NodoAST;
  ...
  traducir(ts: TablaSimbolos) {
    ...
    //Validaciones
    if (!control_izq) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando izquierdo de la multiplicacion` }));
      return;
    }
    if (!control_der) {
      Errores.push(new Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando derecho de la multiplicacion` }));
      return;
    }
    ...
    switch (tipo) {
      case TIPO_DATO.FLOAT:
        Codigo3D.addComentario('Multiplicacion con resultado double');
        Codigo3D.add(`${temporal} = ${control_izq.temporal} * ${control_der.temporal};`);
        return new Control({ temporal, tipo });
      case TIPO_DATO.INT:
        Codigo3D.addComentario('Multiplicacion con resultado int');
        Codigo3D.add(`${temporal} = ${control_izq.temporal} * ${control_der.temporal};`);
        return new Control({ temporal, tipo });
    }
  }
