...
class Division extends nodoAST_1.NodoAST {
    ...
    traducir(ts) {
        ...
        //Validaciones
        if (!control_izq) {
            errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando izquierdo de la Division` }));
            return;
        }
        if (!control_der) {
            errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se pudo obtener la instancia de control para el operando derecho de la Division` }));
            return;
        }
        //Comprobacion de tipo
        const tipo = this.getTipoResultante(control_izq.tipo, control_der.tipo);
        if (tipo === 8 /* NULL */) {
            errores_1.Errores.push(new error_1.Error({ tipo: 'semantico', linea: this.linea, descripcion: `No se puede realizar una Division con los tipos ${tipos_1.getNombreDeTipo(control_izq.tipo)} y ${tipos_1.getNombreDeTipo(control_der.tipo)}` }));
            return;
        }
        ...
        switch (tipo) {
            case 7 /* FLOAT */:
                codigo3D_1.Codigo3D.addComentario('Division con resultado double');
                codigo3D_1.Codigo3D.add(`${temporal} = ${control_izq.temporal} / ${control_der.temporal};`);
                return new control_1.Control({ temporal, tipo });
            case 6 /* INT */:
                codigo3D_1.Codigo3D.addComentario('Division con resultado int');
                codigo3D_1.Codigo3D.add(`${temporal} = ${control_izq.temporal} / ${control_der.temporal};`);
